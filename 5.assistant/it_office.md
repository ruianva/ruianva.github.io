...
- 禁止其他同事`直接通过线缆`方式 接入打印机服务器，因为线缆的插拔很容易导致机械故障；

- 通过一台公用电脑统一安装驱动并共享出来，这样一个网络内的所有同事都可以方便快捷的安装。
  
- 所有打印机都`设置了物理IP`，并以LAN方式接入（以WLAN很不稳定，迫不得已不要使用）。

  - 对于没有网口的打印机，要在线缆上注明`勿插拔`标示；
