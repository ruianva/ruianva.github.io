...
本文基于惟校IT建设的单元划分按照`行政/教辅/教务` 三个层级来进行规划、讨论；

以`nonaggressive/黑盒模式`（最小化改变当前现状方式提高业务提升）进行整改，提高整体运转效率；

IT建设初期应以`行政`作为第一优先级，此块在未优化前往往消耗教师职工最大精力，借鉴主流的`财务`/`物业`行业的IT实践经验，利用尽可能的`IT基础设施/规范` 来提高便捷的资源共享以及业务能力。



## 行政

`行政`的职能：服务教职工的物资、管理校园资产(减少资产浪费)；

- 通过资产清单电子表格共享[清单](https://docs.qq.com/sheet/DWGNUdFJId0ZxSG9K?tab=BB08J2)，并按`基础设施`、`教学资源` 、`办公耗材/文具`专职人员定时检视、以便每学期汇报财务/股东时提供合理依据；
  ...
  - `图书馆图书`：由@玉雁 玉雁姐负责，教师申购图书需要提供审批`ISBN`，`申购审批`；
...
`教辅`阶段的是`数字校园`的数据接入通道，通过对学生管理`学生数据`源；
...
- 教务：
  - 5月计划在图书馆安排8台`树莓派`与`显示器`供学生查询使用，禁止学生携带个人电子设备入校；
