...
- 其中一些书籍不能扫条形码的，因为一部分是台湾或者香港的，还有一些是学生捐的，根本就没有条形码。
- 每天借阅数量在100以内；
...
- `编号贴/libraryNo贴` ：用于标志每本书的唯一性，贴在`封底`。
  - 注意：`这个编号和书籍类别无任何关系`，重复的书也要贴，无需考虑顺序；
  - 许多图书馆会将其贴在`第一页`，我建议贴在`封底`是扫描更方便，不需要每次打开才能扫描；
...
- 每到一批新书，使用`libraryNo贴`（目前已有1000编号），把所有的书都贴上；
- 通过[安卓手机app](https://www.cr173.com/soft/768911.html)扫描书籍的`isbn`，粘贴到[表格](https://shimo.im/sheet/UFae1MU8GmUXUKeu/RIDOC)对应`libraryNo`行；
  - [无线扫描枪](https://detail.tmall.com/item.htm?spm=a230r.1.14.1.19ab6042jyQ5rg&id=584801443261&ns=1&abbucket=5&skuId=3947559432368)效率更高，可以马上回传到电脑，`app`要回传粘贴还要整理格式；
...
- 借书：点击触摸屏`借书`->根据引导扫描`学号`->扫描`libraryNo`；

  - 异常：您的学号不在系统中/此书籍编号不在系统中/您的借阅数>2/您有超期未还书籍/此书已被借阅
  - 完成自助式借阅，详见`book.pdf`  

- 还书：点击触摸屏`还书`->根据引导扫描`libraryNo`;
