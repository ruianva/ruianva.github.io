...
本文基于惟校IT建设的单元划分按照`行政/教辅/教务` 三个层级来进行规划；

以`不改变`或`最小化改变`当前现状方式进行调整，以期对单元提高整体运转效率；



- `行政`的职能：提供教职工的`物资`以及`服务`、配合管理校园资产表（以期管理并减少资产浪费）；

- `教辅`的智能：是`数字校园`的数据接入通道，通过对学生管理`学生数据`源；



IT建设初期`行政`，消耗教师职工较大精力，借鉴主流的`财务`/`物业`行业的IT实践经验，利用尽可能的`IT基础设施/规范` 来提高便捷的资源共享以及业务能力；
