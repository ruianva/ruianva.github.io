...
- 配置:
  `cpu: epyc 7351p*2  ram: 224g   sata hdd: 4t*4  ssd: 240g*2   480g*2  `
...
- 教师协同办公服务，目前使用的免费石墨有较大限制，服务器采购后使用Only office服务；
...
报告单，就是学生的学期评价表（7分制评分+建设性评语）：

- 3段：优异、尚可、不达标

- 5段：优、良、常、可、不达标

- 7段：`4分`为及格线，`7分`为满分，以0.5分间隔进行打分；
...
- 关于打分
  ...
  - 最后总评显示不是以0.5为单位，但是最终word导出会统一处理；
